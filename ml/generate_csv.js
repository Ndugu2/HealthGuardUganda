...
const headers = ['id', 'title', 'title_lg', 'icon', 'color', 'steps_json', 'steps_lg_json'];
const rows = [headers.join(',')];

for (const item of DISEASES_DATA) {
  const stepsJson = JSON.stringify(item.steps);
  const stepsLgJson = JSON.stringify(item.steps_lg);
  
  const row = [
    item.id,
    escapeCSVField(item.title),
    escapeCSVField(item.title_lg),
    escapeCSVField(item.icon),
    escapeCSVField(item.color),
    escapeCSVField(stepsJson),
    escapeCSVField(stepsLgJson)
  ];
  rows.push(row.join(','));
}

const csvContent = rows.join('\n') + '\n';
const targetPath = path.join(__dirname, '..', 'src', 'db', 'uganda_diseases.csv');

fs.writeFileSync(targetPath, csvContent, 'utf-8');
console.log(`\u2705 Rich first aid CSV written to: ${targetPath}`);
console.log(`   ${DISEASES_DATA.length} ailments with detailed clinical guidelines`);
